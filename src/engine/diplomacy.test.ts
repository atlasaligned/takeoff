...
describe('treaty tree', () => {
  ...
  it('gates on prereqs, risk fear and cash', () => {
    ...
    expect(treatyBlocked(state, state.labs.helios, 'global-pause')).toMatch(/needs/);
    expect(treatyBlocked(state, state.labs.helios, 'joint-safety-institute')).toMatch(/needs/);
    state.labs.helios.cash = 1;
    expect(treatyBlocked(state, state.labs.helios, 'transparency-pledge')).toBe('not enough cash');
    state.labs.helios.cash = 5000;
    expect(treatyBlocked(state, state.labs.helios, 'transparency-pledge')).toBeNull();
  });

  it('T1 treaties sign unconditionally and bump trust', () => {
    const state = newGame('helios', 1);
    const trust = state.labs.helios.publicTrust;
    const res = signTreaty(state, state.labs.helios, 'transparency-pledge');
  ...
  it('agreement odds fall when the gatekeeper is far ahead', () => {
    const state = newGame('helios', 1);
    const before = agreementProbability(state, state.labs.helios);
    ...
    const after = agreementProbability(state, state.labs.helios);
  ...
  it('compute cap treaty destroys excess chips for everyone', () => {
    ...
    state.diplomacy.brokeredBy['mutual-inspection'] = 'helios'; // a capstone needs a seat at the talks
    ...
    const res = signTreaty(state, state.labs.helios, 'compute-cap-treaty');
    expect(res.ok).toBe(true);
...
describe('small diplomacy actions', () => {
  it('charm reduces race fear and goes on cooldown', () => {
    ...
    expect(smallDiplomacy(state, state.labs.helios, 'charm', 'us').ok).toBe(true);
    expect(state.govs.us.raceFear).toBeLessThan(fear);
    expect(smallDiplomacy(state, state.labs.helios, 'charm', 'us').ok).toBe(false);
  });
  ...
  it('sounding the alarm raises risk fear everywhere and costs revenue', () => {
    ...
    expect(smallDiplomacy(state, state.labs.helios, 'alarm', null).ok).toBe(true);
